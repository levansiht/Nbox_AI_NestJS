...
export class AuthRepository {
  ...
  async createUser(
    user: Pick<UserType, 'email' | 'name' | 'phoneNumber' | 'roleId' | 'password'>,
  ...
  async createUserIncludeRole(
    user: Pick<UserType, 'email' | 'name' | 'phoneNumber' | 'avatar' | 'roleId' | 'password'>,
  ...
  async findUniqueVerificationCode(
    uniqueValue: { id: number } | { email: string } | { email: string; code: string; type: TypeOfVerificationCodeType },
  ...
  deleteRefreshToken(uniqueObject: { token: string }): Promise<RefreshTokenType> {
    return this.prismaService.refreshToken.delete({
      where: { token: uniqueObject.token },
    });
  }
